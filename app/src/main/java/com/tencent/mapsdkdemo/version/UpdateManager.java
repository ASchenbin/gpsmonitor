package com.tencent.mapsdkdemo.version;
...
import com.tencent.mapsdkdemo.raster.R;
...
public class UpdateManager {
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg){
		...
		final Handler handler = new Handler(){
			public void handleMessage(Message msg) {
				...
				if(msg.what == 1){
					if(mUpdate != null){
						if(isNewVersion(mUpdate.getVersion())){
		...
		new Thread(){
			public void run() {
				...
				try {
					String path = mContext.getResources().getString(R.string.serverurl_version);
					URL url = new URL(path);
					HttpURLConnection conn = (HttpURLConnection) url
							.openConnection();
					conn.setConnectTimeout(5000);
					InputStream is = conn.getInputStream();
					mUpdate = UpdataInfoParser.getUpdataInfo(is);
					msg.what = 1;
	...
	private Runnable mdownApkRunnable = new Runnable() {	
		...
		public void run() {
			try {
				String apkName = "igogo_"+mUpdate.getVersion()+".apk";
				String tmpApk = "igogo_"+mUpdate.getVersion()+".tmp";
				//判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();		
				if(storageState.equals(Environment.MEDIA_MOUNTED)){
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/igogo/";
